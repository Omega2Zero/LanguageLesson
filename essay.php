...
require_once($CFG->dirroot.'/mod/languagelesson/essay_form.php');
...
$url = new moodle_url('/mod/languagelesson/essay.php', array('id'=>$id));
if ($mode !== 'display') {
    $url->param('mode', $mode);
}
$PAGE->set_url($url);
...
if ($attemptid > 0) {
    ...
    $answer = $DB->get_record('languagelesson_answers', array('lessonid' => $lesson->id, 'pageid' => $attempt->pageid));
    ...
    if ($lesson->custom) {
        ...
        $scoreoptions[0] = get_string('nocredit', 'languagelesson');
        $scoreoptions[1] = get_string('credit', 'languagelesson');
    }
}
...
switch ($mode) {
    case 'grade':
        ...
        if (empty($attempt)) {
            print_error('cannotfindattempt', 'languagelesson');
        }
        if (empty($user)) {
            print_error('cannotfinduser', 'languagelesson');
        }
        if (empty($answer)) {
            print_error('cannotfindanswer', 'languagelesson');
        }
        break;
    ...
    case 'update':
        ...
        if ($mform->is_cancelled()) {
            redirect("$CFG->wwwroot/mod/languagelesson/essay.php?id=$cm->id");
        }
        if ($form = $mform->get_data()) {
            if (!$grades = $DB->get_records('languagelesson_grades', array("lessonid"=>$lesson->id, "userid"=>$attempt->userid),
                                                    'completed', '*', $attempt->retry, 1)) {
            ...
            $DB->update_record('languagelesson_attempts', $attempt);

            // Get grade information.
            $grade = current($grades);
            $gradeinfo = languagelesson_grade($lesson, $attempt->retry, $attempt->userid);

            // Set and update.
            $updategrade = new stdClass();
            $updategrade->id = $grade->id;
            $updategrade->grade = $gradeinfo->grade;
            $DB->update_record('languagelesson_grades', $updategrade);
            // Log it.
            add_to_log($course->id, 'languagelesson', 'update grade', "essay.php?id=$cm->id", $lesson->name, $cm->id);

            $lesson->add_message(get_string('changessaved'), 'notifysuccess');

            // Update central gradebook.
            languagelesson_update_grades($lesson, $grade->userid);

            redirect(new moodle_url('/mod/languagelesson/essay.php', array('id'=>$cm->id)));
        } else {
    ...
    case 'email':
        ...
        if (!$attempts = $DB->get_records_select('languagelesson_attempts', "pageid $usql".$queryadd, $params)) {
            print_error('nooneansweredthisquestion', 'languagelesson');
        }
        // Get the answers.
        list($answerusql, $parameters) = $DB->get_in_or_equal(array_keys($pages));
        array_unshift($parameters, $lesson->id);
        if (!$answers = $DB->get_records_select('languagelesson_answers', "lessonid = ? AND pageid $answerusql",
                                                            $parameters, '', 'pageid, score')) {
            print_error('cannotfindanswer', 'languagelesson');
        }
        ...
        foreach ($attempts as $attempt) {
            ...
            if ($essayinfo->graded && !$essayinfo->sent) {
                ...
                // Set the grade.
                $grades = $DB->get_records('languagelesson_grades', array("lessonid"=>$lesson->id, "userid"=>$attempt->userid),
                                                        'completed', '*', $attempt->retry, 1);
                $grade  = current($grades);
                ...
                $message  = get_string('essayemailmessage2', 'languagelesson', $a);
                $plaintext = format_text_email($message, FORMAT_HTML);

                // Subject.
                $subject = get_string('essayemailsubject', 'languagelesson', format_string($pages[$attempt->pageid]->title, true));
                ...
                $eventdata->component = 'mod_languagelesson';
                ...
                $DB->update_record('languagelesson_attempts', $attempt);
                // Log it.
                add_to_log($course->id, 'languagelesson', 'update email essay grade', "essay.php?id=$cm->id",
                        format_string($pages[$attempt->pageid]->title, true).': '.fullname($users[$attempt->userid]), $cm->id);
            }
        }
        $lesson->add_message(get_string('emailsuccess', 'languagelesson'), 'notifysuccess');
        redirect(new moodle_url('/mod/languagelesson/essay.php', array('id'=>$cm->id)));
        break;
    ...
    default:
        ...
        if (count($pages) > 0) {
            $params = array ("lessonid" => $lesson->id, "qtype" => LL_ESSAY);
            // Get only the attempts that are in response to essay questions.
            list($usql, $parameters) = $DB->get_in_or_equal(array_keys($pages));
            if ($essayattempts = $DB->get_records_select('languagelesson_attempts', 'pageid '.$usql, $parameters)) {
                ...
                if (!$users = $DB->get_records_sql($sql, $params)) {
                    ...
                    $lesson->add_message(get_string('noonehasanswered', 'languagelesson'));
                }
            } else {
                $mode = 'none'; // Not displaying anything.
                $lesson->add_message(get_string('noonehasanswered', 'languagelesson'));
            ...
            $mode = 'none'; // Not displaying anything.
            $lesson->add_message(get_string('noessayquestionsfound', 'languagelesson'));
        }
        break;
}
// Log it.
add_to_log($course->id, 'languagelesson', 'view grade', "essay.php?id=$cm->id",
                    get_string('manualgrading', 'languagelesson'), $cm->id);
...
switch ($mode) {
    case 'display':
        ...
        $table->head = array(get_string('name'), get_string('essays', 'languagelesson'), get_string('email', 'languagelesson'));
        ...
        foreach (array_keys($studentessays) as $userid) {
            ...
            $attempts = $DB->count_records('languagelesson_grades', array('userid'=>$userid, 'lessonid'=>$lesson->id));
            ...
            foreach ($studentessays[$userid] as $page => $tries) {
                ...
                foreach ($tries as $try) {
                    ...
                    // Link for each essay.
                    $url = new moodle_url('/mod/languagelesson/essay.php', array('id'=>$cm->id, 'mode'=>'grade',
                                                'attemptid'=>$essay->id, 'sesskey'=>sesskey()));
                    $attributes = array();
                    // Different colors for all the states of an essay (graded, if sent, not graded).
                    if (!$essayinfo->graded) {
                        $attributes['class'] = "graded";
                    } else if (!$essayinfo->sent) {
                        $attributes['class'] = "sent";
                    } else {
                        $attributes['class'] = "ungraded";
                    }
                    $essaylinks[] = html_writer::link($url, userdate($essay->timeseen,
                            get_string('strftimedatetime')).' '.format_string($pages[$essay->pageid]->title, true), $attributes);
                }
            }
            // Email link for this user.
            $url = new moodle_url('/mod/languagelesson/essay.php', array('id'=>$cm->id, 'mode'=>'email',
                                                                            'userid'=>$userid, 'sesskey'=>sesskey()));
            $emaillink = html_writer::link($url, get_string('emailgradedessays', 'languagelesson'));

            $table->data[] = array($OUTPUT->user_picture($users[$userid], array('courseid' => $course->id)).$studentname,
                                                                                implode("<br />", $essaylinks), $emaillink);
        }

        // Email link for all users.
        $url = new moodle_url('/mod/languagelesson/essay.php', array('id'=>$cm->id, 'mode'=>'email', 'sesskey'=>sesskey()));
        $emailalllink = html_writer::link($url, get_string('emailallgradedessays', 'languagelesson'));
