...
require_once($CFG->dirroot.'/mod/languagelesson/import_form.php');
require_once($CFG->dirroot.'/mod/languagelesson/format.php');  // Parent class.

$id     = required_param('id', PARAM_INT);         // Course Module ID.
$pageid = optional_param('pageid', '', PARAM_INT); // Page ID.

$PAGE->set_url('/mod/languagelesson/import.php', array('id'=>$id, 'pageid'=>$pageid));
...
$mform = new languagelesson_import_form(null, array('formats'=>languagelesson_get_import_export_formats('import')));
...
if ($data = $mform->get_data()) {
    ...
    if (! $format->importpreprocess()) {
                print_error('preprocesserror', 'languagelesson');
    }

    // Process the uploaded file.
    if (! $format->importprocess($importfile, $lesson, $pageid)) {
                print_error('processerror', 'languagelesson');
    }

    // In case anything needs to be done after.
    if (! $format->importpostprocess($lesson)) {
                print_error('postprocesserror', 'languagelesson');
    }
