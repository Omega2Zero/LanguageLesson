...
require_once($CFG->dirroot.'/mod/languagelesson/importpptlib.php');

$id     = required_param('id', PARAM_INT);         // Course Module ID
$pageid = optional_param('pageid', '', PARAM_INT); // Page ID

$url = new moodle_url('/mod/languagelesson/importppt.php', array('id'=>$id));
if ($pageid !== '') {
...
$strimportppt = get_string("importppt", "languagelesson");
$strlessons = get_string("modulenameplural", "languagelesson");

$data = new stdClass;
$data->id = $cm->id;
$data->pageid = $pageid;
$mform = new languagelesson_importppt_form();
$mform->set_data($data);

if ($data = $mform->get_data()) {
    $manager = languagelesson_page_type_manager::get($lesson);
    if (!$filename = $mform->get_new_filename('pptzip')) {
        print_error('invalidfile', 'languagelesson');
    }
    if (!$package = $mform->save_stored_file('pptzip', $context->id, 'mod_languagelesson', 'ppt_imports', $lesson->id, '/', $filename, true)) {
        print_error('unabletosavefile', 'languagelesson');
    }
    // extract package content
    $packer = get_file_packer('application/zip');
    $package->extract_to_storage($packer, $context->id, 'mod_languagelesson', 'imported_files', $lesson->id, '/');

    $fs = get_file_storage();
    if ($files = $fs->get_area_files($context->id, 'mod_languagelesson', 'imported_files', $lesson->id)) {
        ...
        foreach ($files as $key=>$file) {
            ...
            for($i = 0; $i < $countmatches; $i++) { // go through all of our div matches
                ...
                $class = languagelesson_importppt_isolate_class($matches[1][$i]); // first step in isolating the class
                ...
                switch ($class) {
                    ...
                    case 'B3':
                    case 'B4':
                        $page->contents[] = languagelesson_importppt_build_list($matches, '<ul>', $i, 0);  // this is a recursive function that will grab all the bullets and rebuild the list in html
                        break;
                    default:
        ...
        $branchtables = languagelesson_create_objects($pages, $lesson->id);
        ...
        if (empty($pageid)) { // adding it to the top of the lesson
            ...
            if (!$nextpageid = $DB->get_field('languagelesson_pages', 'id', array('prevpageid' => 0, 'lessonid' => $lesson->id))) {
                $nextpageid = 0;
            }
        } else {
            // going after an actual page
            $prevpageid = $pageid;
            $nextpageid = $DB->get_field('languagelesson_pages', 'nextpageid', array('id' => $pageid));
        }
        ...
        foreach ($branchtables as $branchtable) {
            ...
            // insert the page
            $id = $DB->insert_record('languagelesson_pages', $branchtable->page);

            if (!empty($branchtable->page->images)) {
                $changes = array('contextid'=>$context->id, 'component'=>'mod_languagelesson', 'filearea'=>'page_contents', 'itemid'=>$id, 'timemodified'=>time());
            ...
            if ($prevpageid != 0) {  // if not the first page
                $DB->set_field("languagelesson_pages", "nextpageid", $id, array("id" => $prevpageid));
            }
        ...
        if ($nextpageid != 0) {  // if the next page is not the end of lesson
            $DB->set_field("languagelesson_pages", "prevpageid", $id, array("id" => $nextpageid));
        }
    }

    // Remove all unzipped files!
    $fs->delete_area_files($context->id, 'mod_languagelesson', 'imported_files', $lesson->id);

    redirect("$CFG->wwwroot/mod/$modname/view.php?id=$cm->id", get_string('pptsuccessfullimport', 'languagelesson'), 5);
}
...
// Print upload form
echo $OUTPUT->heading_with_help($strimportppt, 'importppt', 'languagelesson');
