...
$PAGE->set_url('/mod/languagelesson/index.php', array('id'=>$id));
...
$view_event = \mod_languagelesson\event\course_module_instance_list_viewed::create(array(
...
foreach ($lessons as $lesson) {
    ...
    if ($usesections) {
        if (has_capability('mod/languagelesson:manage', $context)) {
            $gradevalue = $lesson->grade;
        } else {
            // It's a student, show their grade.
            $gradevalue = 0;
            if ($return = languagelesson_get_user_grades($lesson, $USER->id)) {
                $gradevalue = $return[$USER->id]->rawgrade;
            }
        }
        $table->data[] = array (get_section_name($course, $sections[$lesson->section]), $link, $gradevalue, $due);
    } else {
