...
$url = new moodle_url('/mod/languagelesson/highscores.php', array('id'=>$id));
if ($mode !== '') {
    $url->param('mode', $mode);
}
if ($link !== 0) {
...
switch ($mode) {
    ...
    case 'save':
        if (confirm_sesskey() and $form = data_submitted($CFG->wwwroot.'/mod/languagelesson/view.php')) {
            $name = trim(optional_param('name', '', PARAM_CLEAN));

            // Make sure it is not empty.
            if (empty($name)) {
                $lesson->add_message(get_string('missingname', 'languagelesson'));
                $mode = 'add';
                break;
            }
            ...
            foreach ($filterwords as $filterword) {
                if (strstr($name, $filterword)) {
                    $lesson->add_message(get_string('namereject', 'languagelesson'));
                    $mode = 'add';
                    break;
                }
            }
            ...
            if (!$grades = $DB->get_records_select('languagelesson_grades', "lessonid = :lessonid", $params, 'completed')) {
                print_error('cannotfindfirstgrade', 'languagelesson');
            }

            if (!$newgrade = $DB->get_record_sql("SELECT *
                                               FROM {languagelesson_grades}
                                              WHERE lessonid = :lessonid
                                                AND userid = :userid
                                           ORDER BY completed DESC", $params, true)) {
                print_error('cannotfindnewestgrade', 'languagelesson');
            }

            // Check for multiple submissions.
            if ($DB->record_exists('languagelesson_high_scores', array('gradeid' => $newgrade->id))) {
                print_error('onpostperpage', 'languagelesson');
            }

            // Find out if we need to delete any records.
            if ($highscores = $DB->get_records_sql("SELECT h.*, g.grade
                                                 FROM {languagelesson_grades} g, {languagelesson_high_scores} h
                    ...
                    if ($flag) {
                        ...
                        foreach ($highscores as $highscore) {
                            if ($highscore->grade == $lowscore) {
                                $DB->delete_records('languagelesson_high_scores', array('id' => $highscore->id));
                            }
                        }
                    }
                }
            ...
            $DB->insert_record('languagelesson_high_scores', $newhighscore);

            // Log it.
            add_to_log($course->id, 'languagelesson', 'update highscores', "highscores.php?id=$cm->id", $name, $cm->id);

            $lesson->add_message(get_string('postsuccess', 'languagelesson'), 'notifysuccess');
            redirect("$CFG->wwwroot/mod/languagelesson/highscores.php?id=$cm->id&amp;link=1");
        } else {
            print_error('invalidformdata');
        }
        break;
...
// Log it.
add_to_log($course->id, 'languagelesson', 'view highscores', "highscores.php?id=$cm->id", $lesson->name, $cm->id);
...
switch ($mode) {
    ...
    default:
        ...
        echo $OUTPUT->heading(get_string("topscorestitle", "languagelesson", $lesson->maxhighscores), 4);

        if (!$highscores = $DB->get_records_select("languagelesson_high_scores", "lessonid = :lessonid", $params)) {
            echo $OUTPUT->heading(get_string("nohighscores", "languagelesson"), 3);
        } else {
            foreach ($highscores as $highscore) {
            ...
            $table->head = array(get_string("rank", "languagelesson"), get_string('name'), get_string("scores", "languagelesson"));
        ...
        if (!has_capability('mod/languagelesson:manage', $context)) {  // Teachers don't need the links.
            echo $OUTPUT->box_start('mdl-align');
            echo $OUTPUT->box_start('lessonbutton standardbutton');
            if ($link) {
                echo html_writer::link(new moodle_url('/course/view.php',
                                                      array('id'=>$course->id)), get_string("returntocourse", "languagelesson"));
            } else {
                echo html_writer::link(new moodle_url('/course/view.php',
                                                      array('id'=>$course->id)), get_string("cancel", "languagelesson")). ' ';
                echo html_writer::link(new moodle_url('/mod/languagelesson/view.php',
                                            array('id'=>$cm->id, 'viewed'=>'1')), get_string("startlesson", "languagelesson"));
            }
            echo $OUTPUT->box_end();
            echo $OUTPUT->box_end();
        }
