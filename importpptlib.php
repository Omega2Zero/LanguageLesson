 ...
 * @return string
 */
function languagelesson_importppt_build_list(array &$matches, $list, &$i, $depth) {
    while($i < count($matches[1])) {

        $class = languagelesson_importppt_isolate_class($matches[1][$i]);
        ...
        if (strstr($class, 'B')) {  // make sure we are still working with bullet classes
            ...
            if ($this_depth > $depth) {
                ...
                $list .= '<ul>';
                $list = languagelesson_importppt_build_list($matches, $list, $i, $this_depth);
                // once we return back, should go to the start of the while
                continue;
            }
            // no depth changes, so add the match to our list
            if ($cleanstring = languagelesson_importppt_clean_text($matches[3][$i])) {
                $list .= '<li>'.languagelesson_importppt_clean_text($matches[3][$i]).'</li>';
            }
            $i++;
        } else {
 ...
 * @return string
 */
function languagelesson_importppt_isolate_class($string) {
 ...
 * @return string
 */
function languagelesson_importppt_clean_text($string) {
 ...
 * @return array
 */
function languagelesson_create_objects($pageobjects, $lessonid) {
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class languagelesson_importppt_form extends moodleform {
